package mx.ecosur.multigame.oculto
{
    ...
    import mx.ecosur.multigame.oculto.entity.*;
	import mx.ecosur.multigame.oculto.enum.*
	import mx.ecosur.multigame.oculto.token.*;
	...
	public class OcultoGameController
	{
		
	    // visual components
	    private var _gameWindow:OcultoWindow
        ...
        private var _currentPlayer:OcultoPlayer;
        private var _players:ArrayCollection;
        private var _game:OcultoGame;
        ...
        private var _executingMove:OcultoMove;
        ...
        public function OcultoGameController (gameWindow:OcultoWindow) 
        {
        	
        	// set private references
        	_gameWindow = gameWindow;
        	_currentPlayer = OcultoPlayer (gameWindow.currentPlayer);
            _game = OcultoGame(gameWindow.currentGame);
            _gameId = _game.id; 
        ...
        public function initializeTokenStorePanel (tokenStorePanel:Panel):void {
        	var _store:OcultoTokenStore = OcultoTokenStore(
        ...
        public function dragEnterBoardCell(evt:DragEvent):void{
            ...
            if (evt.dragSource.hasFormat("token")){
                ...
                var token:OcultoToken = OcultoToken(evt.dragSource.dataForFormat("token"));
        ...
        private function validateMove(boardCell:RoundCell, token:OcultoToken):Boolean
        {
        ...
        private function startMove(evt:MouseEvent):void{
            ...
            if (!_isMoving && _isTurn){
                ...
                var token:OcultoToken = OcultoToken(evt.currentTarget);
        ...
        private function gotoMove(event:DynamicEvent):void{
            ...
            var move:OcultoMove = OcultoMove(event.move);
            
            // if move is before the currently selected move then iterate
            // back over the moves transforming the board
            // else iterate forward
            if(move.id < OcultoMove(_moves[_selectedMoveInd]).id){
                do{
                    undoMove(OcultoMove(_moves[_selectedMoveInd]));
                    _selectedMoveInd --;                    
                }while(move.id < OcultoMove(_moves[_selectedMoveInd]).id 
                        && _selectedMoveInd > 0);
            }else if (move.id > OcultoMove(_moves[_selectedMoveInd]).id 
                        && _selectedMoveInd < _moves.length){
                do{
                    doMove(OcultoMove(_moves[_selectedMoveInd + 1]));
                    _selectedMoveInd ++;
                } while (move.id > OcultoMove(_moves[_selectedMoveInd]).id 
                    && _selectedMoveInd < _moves.length);
            }
        ...
        public function dragDropCell(evt:DragEvent):void{
            ...
            if (evt.dragSource.hasFormat("token")){
                ...
                // do move in backend
                var move:OcultoMove = new OcultoMove();
        ...
        private function processMessage(event:DynamicEvent):void {
            ...
            switch (gameEvent) {
                ...
                case GameEvent.CHAT:
                    ...
                    break;
                case GameEvent.END:
                    game = OcultoGame(message.body);
                    handleGameEnd (game);
                    break;
                case GameEvent.MOVE_COMPLETE:
                    var move:OcultoMove = OcultoMove(message.body);
        ...
        private function gameServiceResultHandler(event:ResultEvent):void{
            ...
            switch (call.operation){
                ...
                case GAME_SERVICE_GET_PLAYERS_OP:
                    updatePlayers(OcultoGame (event.result));
                    break;
        ...
        private function initTurn():void{             
            if (_gameWindow.currentState == "PLAY")
            {
         ...
         */
        public function updatePlayers(game:OcultoGame):void{
            this._game = game;

            var gamePlayer:OcultoPlayer;
            var i:int;
            
            for (i = 0; i < game.players.length; i++){
                gamePlayer = OcultoPlayer(game.players[i]);
        ...
        private function addMove(move:OcultoMove):void{

            //get last move in game
            var lastMove:OcultoMove = null;
            if (_moves.length > 0){
                lastMove = OcultoMove(_moves[length - 1]);
            }
            ...
            if (lastMove == null || move.id > lastMove.id){
                ...
                var oldMove:OcultoMove;
                for (var i:Number = _moves.length - 1; i >= 0; i--){
                    oldMove = OcultoMove(_moves[i]);
         ...
         * Animates a move
         */
        private function doMove(move:OcultoMove):void{
        	
        	// if was a bad year then nothing to do
        	if(move.badYear){
        		_gameWindow.moveViewer.selectedMove = OcultoMove(_moves[_selectedMoveInd + 1]);
        		return
        	}
        ...
        private function endDoMove(event:EffectEvent):void{
            ...
            var token:OcultoToken = OcultoToken(AnimateProperty(event.currentTarget).target);
            ...
            if (_moves [_selectedMoveInd] != null && _selectedMoveInd > 0) {
                var move:OcultoMove = OcultoMove(_moves[_selectedMoveInd])           
                _gameWindow.moveViewer.selectedMove = move; 
            }            
        }
        
        private function undoMove(move:OcultoMove):void{
            ...
            //define destination
        	var playerBtn:Button = _gameWindow.playersViewer.getPlayerButton(OcultoPlayer(move.player));
            ...
            //create new token
            var token:OcultoToken = new OcultoToken();
            token.cell = move.destinationCell;
            token.width = endSize;
            token.height = endSize;
            _gameWindow.animateLayer.addChild(token);
        ...
        private function endUndoMove(event:EffectEvent):void {
            ...
            if (_selectedMoveInd >= 0)
                _gameWindow.moveViewer.selectedMove = OcultoMove(_moves[_selectedMoveInd]);
        } 
        ...
        private function handleGameEnd (game:OcultoGame):void {
        	var i:int;
