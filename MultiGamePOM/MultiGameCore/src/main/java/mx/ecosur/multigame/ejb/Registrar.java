...
public class Registrar implements RegistrarRemote, RegistrarLocal {
	...
	public Agent registerAgent (Game game, Player registrant, Agent agent) 
		throws InvalidRegistrationException 
	{
		if (!em.contains(game))
			game = em.find(Game.class, game.getId());
		...
		registrant = locatePlayer(registrant.getName());		
		agent.initialize (game, registrant, agent.getColor());
		
		if (!game.getPlayers().contains(agent)) {
			 ...
			 */
			List<Color> availColors = getAvailableColors(game);
			boolean colorAvailable = availColors.contains(agent.getColor());
